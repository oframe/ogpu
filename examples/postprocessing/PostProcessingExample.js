...
export class PostProcessingExample {
    ...
    async init(el) {
        ...
        this.ssr = this.post.addEffect(new SSREffect(this.gpu));
    ...
    initPane() {
        ...
        const perf = this.gui.folder('performance', { expanded: true });
        perf.monitor(this.stats, 'fps', { view: 'graph', min: 0, max: 130 });
        perf.monitor(this.stats, 'frameMs', { label: 'frame-ms' });
    ...
    update = ({ deltaTime = 0 } = {}) => {
        if (deltaTime > 0) {
            this.stats.fps += (1 / deltaTime - this.stats.fps) * 0.05;
            this.stats.frameMs += (deltaTime * 1000 - this.stats.frameMs) * 0.05;
        }
