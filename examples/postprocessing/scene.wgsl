// Post-stack scene shader: linear-HDR color at @location(0), world-space
// normal at @location(1) (composer MRT contract). No gamma here — the final
// pass owns the linear→sRGB boundary.
...
fn fs(in: VertexOutput) -> FragOut {
  ...
  var col = uniforms.uColor * (key + hemi) + vec3f(rim) * uniforms.uColor;
  col += uniforms.uEmissive * uniforms.uEmissiveIntensity;
