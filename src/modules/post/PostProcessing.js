...
// Halton low-discrepancy sequence — the standard TAA jitter pattern.
...
export class PostProcessing {
    ...
    render({ scene, camera } = {}) {
        ...
        // Sub-pixel Halton jitter for temporal effects (TAA): applied to the
        // projection for the WHOLE composer frame — depth and every effect see
        // consistent matrices — and removed before returning to the app.
        let jx = 0;
        let jy = 0;
        const wantsJitter = this.enabled && camera && this.effects.some((e) => e.enabled && e.needsJitter);
        if (wantsJitter) {
            const idx = (this.frameIndex % 16) + 1;
            jx = ((halton(idx, 2) - 0.5) * 2) / this.sceneTarget.width;
            jy = ((halton(idx, 3) - 0.5) * 2) / this.sceneTarget.height;
            camera.projectionMatrix[8] += jx;
            camera.projectionMatrix[9] += jy;
        }
        this.jitter = [jx, jy];
