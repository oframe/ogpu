...
- **Offline WGSL validation.** `npm run validate:shaders` checks every `src/**/*.wgsl` with [naga](https://github.com/gfx-rs/wgpu/tree/trunk/naga), the wgpu reference compiler — so an agent can verify shader edits without a browser. naga isn't bundled; install it with `brew install naga-cli` (or `cargo install naga-cli`). The script exits with code 2 if it's missing.
